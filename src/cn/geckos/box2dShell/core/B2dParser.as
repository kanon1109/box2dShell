package cn.geckos.box2dShell.core
{
...
public class B2dParser 
{
	...
	{
		if (!str) return null;
		var arr:Array = B2dParser.decodeBody(str, b2dShell);
		B2dParser.decodeJoint(str, b2dShell);
		return arr;
	}
		...
		for (var key:String in obj.bodyData)
		{
			...
			if (o && o.shape)
			{
				if (o.shape.type == "poly")
					body = B2dParser.decodePolyBody(o.shape, b2dShell);
				else if (o.shape.type == "circle")
					body = B2dParser.decodeCircleBody(o.shape, b2dShell);
				//显示对象
				body.SetAngle(B2dUtil.dgs2rds(o.shape.rotation));
				arr.push(body);
			}
		}
		return arr;
		...
		for (var key:String in obj.jointData)
		{
			...
			if (o && o.joint)
			{
				if (o.joint.jointType == "revolute")
					B2dParser.decodeRevoluteJoint(o.joint.jointData, b2dShell);
				if (o.joint.jointType == "distance")
					B2dParser.decodeDistanceJoint(o.joint.jointData, b2dShell);
				if (o.joint.jointType == "friction")
					B2dParser.decodeFrictionJoint(o.joint.jointData, b2dShell);
				if (o.joint.jointType == "pulley")
					B2dParser.decodePulleyJoint(o.joint.jointData, b2dShell);
				if (o.joint.jointType == "weld")
					B2dParser.decodeWeldJoint(o.joint.jointData, b2dShell);	
			}
		}
		return arr;
	}
	...
	public static function encode(b2dShell:B2dShell):String
	{
		...
		for (var i:int = 0; i < length; i += 1)
		{
			...
			o.shape = B2dParser.encodeBody(body);
		}
		...
		for (i = 0; i < length; i += 1)
		{
			...
			o.joint = B2dParser.encodeJoint(joint, b2dShell);
		}
	...
	public static function encodeJoint(joint:b2Joint, b2dShell:B2dShell):Object
	{
		...
		if (joint is b2RevoluteJoint)
		{
			...
			o.jointData = B2dParser.encodeRevoluteJoint(b2RevoluteJoint(joint), b2dShell);
		}
		else if (joint is b2DistanceJoint)
		{
			//编码距离关节
			o.jointType = "distance";
			o.jointData = B2dParser.encodeDistanceJoint(b2DistanceJoint(joint), b2dShell);
		}
		else if (joint is b2FrictionJoint)
		{
			//编码摩擦关节
			o.jointType = "friction";
			o.jointData = B2dParser.encodeFrictionJoint(b2FrictionJoint(joint), b2dShell);
		}
		else if (joint is b2PulleyJoint)
		{
			//编码滑轮关节
			o.jointType = "pulley";
			o.jointData = B2dParser.encodePulleyJoint(b2PulleyJoint(joint), b2dShell);
		}
		else if (joint is b2WeldJoint)
		{
			//编码焊接关节
			o.jointType = "Weld";
			o.jointData = B2dParser.encodeWeldJoint(b2WeldJoint(joint), b2dShell);
		}
		return o;
	}
	...
	public static function encodeBody(body:b2Body):Object
	{
		...
		if (body.GetUserData())
		{
			...
			shapeObj.rotation = B2dUtil.rds2dgs(body.GetAngle());
