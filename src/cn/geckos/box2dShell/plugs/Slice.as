package cn.geckos.box2dShell.plugs
{
	...
	public class Slice
	{
		...
		//存放不需要切割的刚体
		private var dontSliceDict:Dictionary;
		...
		private function laserFired(fixture:b2Fixture, point:b2Vec2, normal:b2Vec2, fraction:Number):Number
		{
			...
			if (this.dontSliceDict && this.dontSliceDict[affectedBody]) return 1;
			//获取刚体图形
		...
		private function drawCircle(point:b2Vec2, color:Number):void
		{
			this.canvas.graphics.lineStyle(2, color);
			this.canvas.graphics.drawCircle(point.x * B2dShell.CONVERSION, point.y * B2dShell.CONVERSION, 3);
		}
		 ...
		 */
		public function addDontSliceDict(body:b2Body):void
		{
			if (!this.dontSliceDict)
				this.dontSliceDict = new Dictionary();
			this.dontSliceDict[body] = body;
		}
		 ...
		 * @param	body  需要被切割的刚体
		 */
		public function deleletDontSliceDict(body:b2Body):void
		{
			if (!this.dontSliceDict) return;
			delete this.dontSliceDict[body];
		}
